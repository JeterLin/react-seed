import { AxiosServiceType, BaseRespType,  ServiceHandlerType, ToServiceType } from '../types';
// 只处理响应状态为200或304的响应数据，并把service每一个函数的返回类型由Promise<AxiosResponse<BaseRespType>>转化为Promise<BaseRespType>返回
export function toService<T extends object>(service: AxiosServiceType<T>): ToServiceType<T> {
